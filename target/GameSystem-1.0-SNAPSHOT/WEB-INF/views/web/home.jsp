        ...
        <div class="container">
            <div class="row">
                <div class="col-8">
                    <div class="row">
                        <div class="col-12">
                            <a href="${pageContext.request.contextPath}/bai-viet?id=${findAllByHighlight[0].id}" class="nav-link p-0">
                                <img src="<c:url value='/template/photo/'/>${findAllByHighlight[0].photo}" alt="san-pham-1" class="img img-fluid" width="100%">
                            </a>
                            <h2 class="mb-0">
                                <a href="${pageContext.request.contextPath}/bai-viet?id=${findAllByHighlight[0].id}" class="text-dark nav-link pl-0">${findAllByHighlight[0].title }</a>
                            </h2>
                            <div class="mb-3">
                                <span class="ml-0"><i class="fas fa-certificate"></i> ${findAllByHighlight[0].category.name }</span>
                                <span class="ml-3"><i class="far fa-clock"></i> ${findAllByHighlight[0].createDate }</span>
                                <span class="ml-3"><i class="fas fa-eye"></i> ${findAllByHighlight[0].view}</span>
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="row">
                            	<c:forEach items="${findAllByHighlight}" var="articleHighlight" begin="1">
                            		<%-- <c:if test="${articleHighlight.index > 0 }"> --%>
		                                <div class="col-4">
		                                    <a href="${pageContext.request.contextPath}/bai-viet?id=${articleHighlight.id}" class="nav-link p-0">
		                                        <img src="<c:url value='/template/photo/'/>${articleHighlight.photo }" alt="san-pham-1" class="img img-fluid" width="100%">
		                                    </a>
		                                    <h5 class="mb-0">
		                                        <a href="${pageContext.request.contextPath}/bai-viet?id=${articleHighlight.id}" class="text-dark nav-link p-0">${articleHighlight.title }</a>
		                                    </h5>
		                                    <div>
		                                        <span><i class="far fa-clock"></i> ${articleHighlight.createDate }</span>
		                                        <span class="ml-3"><i class="fas fa-eye"></i> ${articleHighlight.view }</span>
		                                    </div>
		                                </div>
	                                <%-- </c:if> --%>
                                </c:forEach>                              
